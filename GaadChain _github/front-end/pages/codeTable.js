...
export default function ownerCode() {
    ...
    let isOwner = (account === "0xefaAE999c2F779BA9344A3a51d196601dBb8dDBa");
    ...
    async function viewPeerCodes() {
        ...
        try {
            const codes = await waste.listPeerTransfers();
            console.log(codes)
            setCodes(codes);
        } catch (e) {
            console.log(e);
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {codeArray.map((code, i) => {
                                    return (
                                        <tr>
                                            <td className="px-6 py-4 whitespace-nowrap">{code ? i + 1 : ""}</td>
                                            <td className="px-6 py-4 whitespace-nowrap">{code}</td>
                                            <td className="px-6 py-4 whitespace-nowrap">{code.substring(0, 6)}</td>
                                            <td className="px-6 py-4 whitespace-nowrap">{allotCategory(code.substring(6, 7))}</td>
                                            <td className="px-6 py-4 whitespace-nowrap">{code.substring(7, 13)}</td>
                                            <td className="px-6 py-4 whitespace-nowrap">{code.substring(13, 20)}</td>
                                            <td className="px-6 py-4 whitespace-nowrap">{allotType(code.substring(20, 21))}</td>
                                        </tr>
                                    )
                                })}
                            </tbody>
